import { API, BACKEND_URL } from "./config.js";
...
function render(items, showWhy) {
  if (!items || items.length === 0) {
    listEl.innerHTML = `<div class="empty">${searchMode ? "No relevant items found." : "Nothing saved yet. Save something above."}</div>`;
    return;
  }
  listEl.innerHTML = "";
  items.forEach((it) => {
    ...
    const img = it.content_type === "image" && it.image_path
      ? `<img src="${API}/files/${it.image_path}" />` : "";
    const preview = it.summary || it.original_text || it.source_url || (it.status === "processing" ? "Understanding…" : "");
    const why = showWhy && it.match_reason ? `<div class="why">why · ${it.match_reason}</div>` : "";
...
$("save-page").onclick = async (e) => {
  ...
  try {
    ...
    await fetch(`${API}/items/url`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ url: tab.url, context_text: tab.title }),
    });
...
$("save-screenshot").onclick = async (e) => {
  ...
  try {
    ...
    await fetch(`${API}/items/image`, { method: "POST", body: fd });
    toast("Saved to Forgot AI ✓");
    loadRecent();
