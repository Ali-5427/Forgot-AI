...
function AppButton({ children, size = "default", onOpenApp }) {
  return (
    <Button
      type="button"
      size={size}
    ...
    <figure className="mx-auto max-w-xl">
      <div className="rounded-xl bg-cream p-4 shadow-soft">
        <div className="flex items-center gap-3 rounded-sm bg-paper px-4 py-3 text-ink">
          <Search className="h-4 w-4 shrink-0 text-ink-muted" strokeWidth={1.75} aria-hidden="true" />
        ...
        <div className="mt-3 rounded-sm bg-paper p-5">
          <p className="text-xs font-medium tracking-wide text-ink-muted">Saved page</p>
    ...
    <div className="forgot-landing paper-grain relative text-ink">
      ...
      <main>
        <section className="mx-auto grid max-w-5xl items-center gap-10 px-5 py-14 sm:px-8 sm:py-20 lg:grid-cols-2 lg:gap-16 lg:py-24">
          <div>
            <p className="hero-enter text-sm font-medium tracking-wide text-ink-muted">
              For people who keep losing things they already saw
            </p>
            <h1 className="font-display hero-enter hero-enter-delay-1 mt-4 text-4xl font-medium leading-tight tracking-tight text-ink sm:text-5xl lg:text-6xl">
          ...
          <figure className="hero-enter hero-enter-delay-4">
            <img
              ...
              width={1152}
              height={864}
              className="outline-ink w-full rounded-xl shadow-soft"
            />
          </figure>
        </section>
                ...
                <Reveal key={step.n}>
                  <li className="lift h-full rounded-xl bg-cream p-6 shadow-soft">
                    <div className="flex items-center justify-between">
                      <span className="flex h-10 w-10 items-center justify-center rounded-md bg-paper text-ink">
                        <step.icon className="h-5 w-5" strokeWidth={1.75} aria-hidden="true" />
                      </span>
                      <span className="font-display text-sm tracking-wide text-ink-muted">{step.n}</span>
                    </div>
                    <h3 className="font-display mt-6 text-xl font-medium tracking-tight text-ink">{step.title}</h3>
              ...
              {SAVED.map((item) => (
                <Reveal key={item.label}>
                  <li className="lift h-full rounded-xl bg-cream p-5 shadow-soft">
                    ...
                    <h3 className="font-display mt-5 text-lg font-medium tracking-tight text-ink">{item.label}</h3>
                    <p className="mt-1 leading-relaxed text-ink-muted">{item.hint}</p>
                  </li>
                </Reveal>
              ))}
            </ul>
        ...
        <section className="border-t border-line">
          <Reveal className="mx-auto max-w-2xl px-5 py-16 sm:px-8 sm:py-20">
            <div className="rounded-xl bg-cream p-6 shadow-soft sm:p-8">
              <span className="flex h-10 w-10 items-center justify-center rounded-md bg-paper text-ink">
                <Puzzle className="h-5 w-5" strokeWidth={1.75} aria-hidden="true" />
              </span>
              <h2 className="font-display mt-5 text-2xl font-medium tracking-tight text-ink sm:text-3xl">
              ...
              <Accordion type="single" collapsible className="border-t border-line">
                {FAQ.map((item) => (
                  <AccordionItem key={item.q} value={item.q}>
                    <AccordionTrigger className="text-base text-ink">{item.q}</AccordionTrigger>
